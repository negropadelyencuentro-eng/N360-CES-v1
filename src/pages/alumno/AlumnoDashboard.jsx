...
// PDFViewer cargado lazy — solo cuando se necesita, no rompe el bundle inicial
  ...
  import("../../components/PDFViewer").catch(() => ({
    default: ({ onClose }) => (
      <div className="fixed inset-0 bg-black z-50 flex flex-col items-center justify-center gap-4 p-6">
        <p className="text-zinc-400 text-sm text-center">No se pudo cargar el visor de PDF en este navegador.</p>
...
const TABS = [
  ...
  },
  {
    ...
    icon: (<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M6.5 6.5h11" /><path d="M6.5 17.5h11" /><path d="M3 9.5h2.5v5H3z" /><path d="M18.5 9.5H21v5h-2.5z" /><path d="M5.5 12h13" /></svg>),
  },
];
...
function CuotaBadge({ status }) {
  if (!status || status === "AL_DIA") return (
    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
      <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block" />Al día
    </span>
  );
  if (status === "VENCIDA") return (
    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-red-500/10 text-red-400 border border-red-500/20">
      <span className="w-1.5 h-1.5 rounded-full bg-red-400 inline-block" />Cuota vencida
    </span>
  );
  if (status === "POR_VENCER") return (
    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-yellow-500/10 text-yellow-400 border border-yellow-500/20">
      <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 inline-block" />Por vencer
    </span>
  );
  return null;
}
...
export default function AlumnoDashboard() {
  ...
  const fetchData = async () => {
    try {
      const [rutinaRes, asistenciasRes] = await Promise.all([
        supabase.from("routines").select("*").eq("student_id", user.id)
          .order("created_at", { ascending: false }).limit(1).maybeSingle(),
        supabase.from("attendances").select("id, date, status").eq("student_id", user.id)
  ...
  const handleDescargar = async () => {
    ...
    setDownloading(true);
    try {
      const { data } = await supabase.storage.from("routines").createSignedUrl(rutina.file_url, 3600);
  ...
  const handleRegistrarAsistencia = async () => {
    ...
    const { error } = await supabase.from("attendances").insert({
      student_id: user.id, gym_id: user.gym_id ?? null, date: TODAY, status: "PRESENTE",
    });
    if (error) {
      ...
      const { data } = await supabase.from("attendances").select("id, date, status")
        .eq("student_id", user.id).order("date", { ascending: false }).limit(30);
  ...
  return (
    <div className="bg-black text-white flex flex-col" style={{minHeight: "100svh"}}>
      {/* Header */}
      <header
        className="flex items-center justify-between px-5 border-b border-zinc-800 sticky top-0 bg-black/90 backdrop-blur-sm z-10"
        style={{ paddingTop: "max(16px, env(safe-area-inset-top))", paddingBottom: "16px" }}
      >
        <Logo variant="full" />
        <button onClick={logout} className="text-zinc-400 hover:text-white text-sm transition-colors flex items-center gap-2 active:opacity-60">
            ...
            {supported && permission !== "granted" && (
              <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-zinc-500 mt-0.5">Recibí avisos de rutinas y vencimientos.</p>
                </div>
                <button onClick={requestAndSubscribe} className="btn-primary text-xs px-3 py-2 flex-shrink-0">Activar</button>
              </div>
            )}
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
              <div className="flex items-center gap-2 mb-1">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="text-zinc-400">
                  <rect x="3" y="4" width="18" height="18" rx="2" /><line x1="3" y1="10" x2="21" y2="10" /><path d="m9 16 2 2 4-4" />
                </svg>
              ...
              {yaRegistroHoy ? (
                ...
                <div>
                  <button onClick={handleRegistrarAsistencia} disabled={registrando}
                    className="w-full py-3.5 bg-white text-black font-semibold rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2 text-sm active:opacity-70"
                  >
                    {registrando ? (
                      <><div className="w-4 h-4 border-2 border-black border-t-transparent rounded-full animate-spin" />Registrando...</>
                    ) : (
                      <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>¡Estoy aquí! — Registrar asistencia</>
                    )}
                  </button>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
              ...
              {rutina ? (
                <div>
                  ...
                  <p className="text-zinc-500 text-xs mb-4">Asignada el {new Date(rutina.created_at).toLocaleDateString("es-AR")}</p>
                  ...
                  >
                    ...
                    {downloading ? "Abriendo..." : "Ver rutina"}
                  </button>
                </div>
              ) : (
                  ...
                  {asistencias.map((a) => (
                    <div key={a.id} className="flex items-center justify-between py-3 border-b border-zinc-800/60 last:border-0">
                      <span className="text-sm text-zinc-300 capitalize">
                        {new Date(a.date + "T12:00:00").toLocaleDateString("es-AR", { weekday: "long", day: "numeric", month: "short" })}
                      </span>
